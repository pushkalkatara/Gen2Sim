...
def deal_with_dof_state(state_tensor, cabinet_dof_num, franka_dof_num, env_num):
    env_dof_sate_list =[]
    new_tensor = torch.zeros((env_num, franka_dof_num + 1, state_tensor.shape[-1]), device = state_tensor.device)
    now = 0

    assert(state_tensor.shape[0] == env_num*(franka_dof_num + torch.tensor(cabinet_dof_num)))

    for i_ in range(env_num):
        new_tensor[i_,0:franka_dof_num] = state_tensor[now:now+franka_dof_num]
        new_tensor[i_, franka_dof_num] = state_tensor[now+franka_dof_num+cabinet_target_id]
        env_dof_sate_list.append(state_tensor[now: now + franka_dof_num + cabinet_dof_num])
        now += franka_dof_num + cabinet_dof_num
        

    return new_tensor

def deal_with_rigid_state(state_tensor, part_id, handle_id, base_id,  cabinet_rigid_num, franka_rigid_num, env_num, env_per_asset):
    ...
    now = 0
    assert(state_tensor.shape[0] == env_num* franka_rigid_num + torch.tensor(cabinet_rigid_num)*env_per_asset)
    for i_ in range(env_num):
        mask[i_,0: franka_rigid_num] = torch.arange(now, now+franka_rigid_num, device = state_tensor.device)
        mask[i_, franka_rigid_num] = now+franka_rigid_num+base_id
        mask[i_, franka_rigid_num+1] = now+franka_rigid_num+part_id
        mask[i_, franka_rigid_num+2] = now+franka_rigid_num+handle_id
        new_tensor[i_,0: franka_rigid_num] = state_tensor[now:now+franka_rigid_num]
        new_tensor[i_, franka_rigid_num] =   state_tensor[now+franka_rigid_num+base_id]
        new_tensor[i_, franka_rigid_num+1] = state_tensor[now+franka_rigid_num+part_id]
        new_tensor[i_, franka_rigid_num+2] = state_tensor[now+franka_rigid_num+handle_id]
        now += franka_rigid_num + cabinet_rigid_num
